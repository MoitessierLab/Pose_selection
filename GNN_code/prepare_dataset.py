...
class generate_datasets:
    ...
    def __getitem__(self, idx):
        ...
        if self.args.graph_as_input is False:
            ...
            if node_features.shape[1] > 30:
                # Features 14: Water (#31)
                if self.args.atom_feature_water is False:
                    node_features = torch.cat([node_features[:, 0:30], node_features[:, 31:]], dim=1)

                # Features 13: HBA/HBD (#29-30)
                if self.args.atom_feature_HBA_HBD is False:
                    node_features = torch.cat([node_features[:, 0:28], node_features[:, 30:]], dim=1)

                # Features 12: Atom formal charge (#26-28)
                if self.args.atom_feature_formal_charge is False:
                    node_features = torch.cat([node_features[:, 0:25], node_features[:, 28:]], dim=1)

                # print("prepare set 102: ", node_features.shape)
                # print("\n", node_features)
                # Features 11: Number of hydrogen (#23-25)
                ...
                # Features 10: Aromaticity (#22)
                if self.args.atom_feature_aromaticity is False:
                    node_features = torch.cat([node_features[:, 0:21], node_features[:, 22:]], dim=1)
                # print("prepare set 113: ", node_features.shape)
                # print("\n", node_features)

                # Feature 9: contribution to TPSA (#21)
                if self.args.atom_feature_TPSA is False:
                    ...
                    node_features = torch.cat([node_features[:, 0:20], node_features[:, 21:]], dim=1)
                # print("prepare set 119: ", node_features.shape)
                # print("\n", node_features)

                # Feature 8: molecular refractivity (#20)
                if self.args.atom_feature_MR is False:
                    node_features = torch.cat([node_features[:, 0:19], node_features[:, 20:]], dim=1)

                # Feature 7: contribution to logP (#19)
                if self.args.atom_feature_logP is False:
                    node_features = torch.cat([node_features[:, 0:18], node_features[:, 19:]], dim=1)

                # Feature 6: hybridization (#16-18)
                if self.args.atom_feature_hybridization is False:
                    node_features = torch.cat([node_features[:, 0:15], node_features[:, 18:]], dim=1)
    ...
    def __getitem__(self, idx):
        ...
        if self.args.graph_as_input is False:
            ...
            if node_features.shape[1] >= 30:
                ...
                #print("356\n", node_features.shape, "\n", node_features)
                # Features 10: Number of hydrogen (#23-25)
                if self.args.atom_feature_number_of_Hs is False:
                    node_features = torch.cat([node_features[:, 0:22], node_features[:, 25:]], dim=1)
