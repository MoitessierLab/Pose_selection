...
def argsParser():
    ...
    parser.add_argument("--mode", help="filter_split_set, train_model, split_good_bad_poses, prepare_complexes, prepare_graphs, train, usage, predict", type=str, default="usage")
    ...
    parser.add_argument("--list_trainset", help="path to data folder", type=str, default='list_trainset.txt')
    parser.add_argument("--list_valset", help="path to data folder", type=str, default='list_valset.txt')
    parser.add_argument("--list_testset", help="path to data folder", type=str, default='list_testset.txt')
...
# ANNE
    parser.add_argument("--CSV_data_dir", help="path to data folder", type=str, default='CSV/')
    parser.add_argument("--CSV_suffix", help=" CSV file name suffix", type=str, default='_good_bad_substract.csv')
    parser.add_argument("--CSV_suffix2", help=" CSV file name suffix", type=str, default='_good_bad_substract.csv')
    parser.add_argument("--model", help="ML model to train: LR, XGBr", type=str, default='none')
    parser.add_argument("--normalize", help="Normalize the dataset, yes or no", type=str, default='yes')
    parser.add_argument("--hyperparameter", help="Percentage of the train set used for HP tuning, 0 for no tuning", type=int, default='100')
    parser.add_argument("--max_depth", help="Maximum depth of a tree", type=int, default='6')
    parser.add_argument("--max_leaves", help="Maximum number of nodes to be added", type=int, default='0')
    parser.add_argument("--min_child_weight", help="Minimum sum of weights of all observations required in a child", type=float, default='1')
    parser.add_argument("--n_estimators", help="Number of trees", type=int, default='100')
    parser.add_argument("--eta", help="learning_rate", type=float, default='0.3')
    parser.add_argument("--gamma", help="minimum loss reduction required to split a node", type=float, default='0')
    parser.add_argument("--reg_alpha", help="L1 regularization term on weights", type=float, default='1')
    parser.add_argument("--reg_lambda", help="L2 regularization term on weights", type=float, default='0')
    parser.add_argument("--tree_method", help="tree_method", type=str, default='hist')

    #parser.add_argument("--max_delta_step", help="max_delta_step", type=int, default='0')
    #parser.add_argument("--subsample", help="subsample", type=int, default='1')

    #parser.add_argument("--seed", help="seed for hp tuning", type=str, default='345')
    parser.add_argument("--maxevals", help="number for models for hyperparameter tuning", type=int, default=50)
    parser.add_argument("--model_name", help="Model file name", type=str, default='latest')
    parser.add_argument("--name", help="Description of the model", type=str, default='')
    parser.add_argument("--plotmetrics", help="Plot feature importances, accuracy, etc.", type=str, default='no')
    parser.add_argument("--set", help="Set to test the accuracy on (Fitted, Astex, or PBDBind)", type=str, default='FittedSet')
    parser.add_argument("--features", help="Feature names to drop", type=str, default='Name,RMSD,Label')
    parser.add_argument("--features_train", help="Feature names to keep", type=str, default='')
    parser.add_argument("--test_on", help="Set to plot the accuracy with", type=str, default='test')
    parser.add_argument("--mismatch", help="Train of mismatched pairs, substractions, additions", type=str, default='pair')
    #parser.add_argument("--plotstuff", help="plotstuff", type=str, default='')
    args = parser.parse_args()

    print('|-------------------------------------------------------------------------------------------------------------------|')
    print('| ML Energy - Anne Labarre and Nic Moitessier                                                                       |')
...
#   ANNE
    if args.mode == 'filter_split_set':
        print('| --CSV_data_dir (path to the CSV data folder):                      %-46s |' % args.CSV_data_dir)
        print('| --energy_threshold (poses above this threshold from min energy):   %-46.0f |' % args.energy_threshold)
        print('| --list_trainset (list of proteins for train set, if provided):     %-46s |' % args.list_trainset)
        print('| --list_valset (list of proteins for validation set):               %-46s |' % args.list_valset)
        print('| --list_testset (list of proteins for test set, if provided):       %-46s |' % args.list_testset)
#   ANNE
    if args.mode == 'train_model':
        print('| --model (model type):                                              %-46s |' % args.model)
        print('| --features (features not used to train the model):                 %-46s |' % str(args.features))
        print('| --CSV_data_dir (path to the CSV data folder):                      %-46s |' % args.CSV_data_dir)
        print('| --list_trainset (list of proteins for train set, if provided):     %-46s |' % args.list_trainset)
        print('| --list_valset (list of proteins for validation set):               %-46s |' % args.list_valset)
        print('| --list_testset (list of proteins for test set, if provided):       %-46s |' % args.list_testset)
        print('| --normalize (was the dataset normalized):                          %-46s |' % args.normalize)
        print('| --hyperparameter (percent of the train set used for tuning, 0 if no tuning): %-36s |' % args.hyperparameter)
#        print('| --seed (seed used for hyperparameter tuning):                      %-46s |' % args.seed)
        print('| --maxevals (number of models for hyp tuning):                      %-46.0f |' % args.maxevals)

#   ANNE
    if args.mode == 'test_model':
        print('| --model (model type):                                              %-46s |' % args.model)
        print('| --model_name (model file name):                                    %-46s |' % args.model_name)
        print('| --set (name of the set the plot the accuracy):                     %-46s |' % args.CSV_data_dir)
        print('| --set (Set to plot the accuracy):                                  %-46s |' % args.set)
        print('| --normalize (was the dataset normalized):                          %-46s |' % args.normalize)


    #print('| --data_dir (path to the data folder - must be pickled):            %-46s |' % args.data_dir)
    #print('| --raw_data_dir (path to the CSV data folder):                      %-46s |' % args.raw_data_dir)
