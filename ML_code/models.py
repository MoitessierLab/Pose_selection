...
# ----------- Hyperparameter tuning
    ...
    if args.hyperparameter == 0:
        ...
        best = {
            'max_depth': 1,
            'gamma': 2.1445289016338402,
            'reg_alpha': 60,
            'reg_lambda': 0.88046665095907639,
            'min_child_weight': 2,
            'n_estimators': 180,
            'random_state': 345}
...
#   model.fit(X, y)
    ...
    model.load_model('models/' + '2025-02-24_10h17-12_XGBr_model.json')
...
# ----------- Saving the model
    ...
    predicted_train = model.predict(train_set_X)
    MAE_train = mean_absolute_error(expected_train, predicted_train)
    print("MAE train set (XGBr): " + str(MAE_train))

    # MAE on the test set
    expected_val = val_set_y
    predicted_val = model.predict(val_set_X)
    MAE_val = mean_absolute_error(expected_val, predicted_val)
    print("MAE val set (XGBr):  " + str(MAE_val))

    # MAE on the test set
    expected_test = test_set_y
    predicted_test = model.predict(test_set_X)
    MAE_test = mean_absolute_error(expected_test, predicted_test)
    print("MAE test set (XGBr):  " + str(MAE_test))

    # Astex = pd.read_csv(pm['dir'] + pm['data_dir'] + 'Astex_top10_combined.csv')
    # Astex_y = Astex['Label']
    # features_to_drop = ['Name', 'RMSD', 'Label']
    # Astex_X = Astex.drop(columns=features_to_drop, axis=1)
    # Astex_X_norm = preprocessing.normalize(Astex_X)
    ...
    # predicted_Astex = model.predict(Astex_X_norm)
    # MAE_Astex = mean_absolute_error(expected_Astex, predicted_Astex)
    # print("MAE Astex set (XGBr):  " + str(MAE_Astex))
